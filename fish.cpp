...
bool Fish::init()
{
	...
	start_path = motion.position;
	end_path = {0, motion.position.y};
...
void Fish::update(float ms)
{
	...
	if (get_bounding_box().x - salmon_posit.x < 0.0) {
...
vec2 Fish::salmon_pos(vec2 salmon) {
	return salmon_posit = salmon;
}
