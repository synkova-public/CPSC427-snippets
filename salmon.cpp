...
void Salmon::update(float ms)
{
	...
	float angleR = -0.03145f + motion.radians;
	float angleL = 0.03145f + motion.radians;
	float angle;
	if (m_is_alive)
	{
		// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
		...
		if (m_keys[GLFW_KEY_DOWN]) {
			...
			move({-cosf(motion.radians) * step, -sinf(motion.radians) * step});
		}
	...
	if (m_light_up_countdown_ms > 0.f)
		...
		// Set the velocity to zero so the salmon does not keep moving even when
...
void Salmon::draw(const mat3& projection)
{
	...
	if (m_is_alive) {
		...
		float color_dead[] = { 1.f, 0.f, 0.f };
		glUniform3fv(color_uloc, 1, color_dead);
	}
	...
	int light_up = 0;
	if (m_has_eaten && m_light_up_countdown_ms > 0.f) {
		light_up = 1;
	} else
			light_up = 0;
...
void Salmon::set_advanced(bool mode) {
	m_advanced = mode;
}
