...
namespace
{
	const size_t MAX_TURTLES = 8; // changed from 15
	const size_t MAX_FISH = 2; // changed from 5
...
bool World::init(vec2 screen)
{
	...
	m_current_speed = 0.3f; // changed form 1.f
	game_mode = BASIC;
...
bool World::update(float elapsed_ms)
{
	...
	for (auto& fish : m_fish) {
		fish.salmon_pos(m_salmon.get_position());
	...
	switch (key) {
		case GLFW_KEY_RIGHT:
			if (action == GLFW_PRESS)
				m_salmon.m_keys[GLFW_KEY_RIGHT] = true;
			if (action == GLFW_RELEASE)
				m_salmon.m_keys[GLFW_KEY_RIGHT] = false;
			break;
		case GLFW_KEY_LEFT:
			if (action == GLFW_PRESS)
				m_salmon.m_keys[GLFW_KEY_LEFT] = true;
			if (action == GLFW_RELEASE)
				m_salmon.m_keys[GLFW_KEY_LEFT] = false;
			break;
		case GLFW_KEY_DOWN:
			if (action == GLFW_PRESS)
				m_salmon.m_keys[GLFW_KEY_DOWN] = true;
			if (action == GLFW_RELEASE)
				m_salmon.m_keys[GLFW_KEY_DOWN] = false;
			break;
		case GLFW_KEY_UP:
			if (action == GLFW_PRESS)
				m_salmon.m_keys[GLFW_KEY_UP] = true;
			if (action == GLFW_RELEASE)
				m_salmon.m_keys[GLFW_KEY_UP] = false;
			break;
		case GLFW_KEY_A:
			game_mode = ADVANCED;
			m_salmon.set_advanced(true);
			break;
		case GLFW_KEY_B:
			game_mode = BASIC;
			m_salmon.set_advanced(false);
			break;
		case GLFW_KEY_SPACE:
			if (game_mode == ADVANCED) {
					if (action == GLFW_PRESS)
						m_salmon.m_keys[GLFW_KEY_SPACE] = true;
					if (action == GLFW_RELEASE)
						m_salmon.m_keys[GLFW_KEY_SPACE] = false;
			}
			break;
	}
...
void World::on_mouse_move(GLFWwindow* window, double xpos, double ypos)
{
	...
	//changed it to just rotate around the x-axis of the salmon
	float x = m_salmon.get_position().x;
	float y = m_salmon.get_position().y;
	x = xpos - x;
	float angle = x * 0.01745f; // 1 degree, trial and error
	m_salmon.set_rotation(angle);

}

// refernces for the collision handling:
//		http://box2d.org/files/GDC2019/ErinCatto_DynamicBVH_GDC2019.pdf
//		https://www.toptal.com/game/video-game-physics-part-ii-collision-detection-for-solid-objects
bool test_overlap_walls(AABB a, AABB b) {
